...
int xdp_prog_simple(struct xdp_md *ctx) {
  ...
  // Verify ETH header contains IPV4 Packet
  struct ethhdr *eth = data;
  if (eth + 1 > data_end) return XDP_PASS; 
  if (eth->h_proto != __constant_htons(ETH_P_IP)) return XDP_PASS;

  // Verify IPV4 header contains TCP Packet
  ...
  // Verify TCP Port
  int16_t dport = ((th->dest & 0xFF) << 8) | ((th->dest & 0xFF00) >> 8);  
  if (dport != 7777) return XDP_PASS;
  ...
  if (payload[0] == 'c' && payload[1] == 'b' && payload[2] == 'r') {
    ...
    bpf_trace_printk(debug, sizeof(debug), diff, dsize, udpEnd);
